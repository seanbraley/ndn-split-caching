...
#ifndef NDN_CONTENT_STORE_IMPL_SPECIAL_H_
#define NDN_CONTENT_STORE_IMPL_SPECIAL_H_
...
class ContentStoreImplSpecial
  : public ContentStore,
    protected ndnSIM::
      trie_with_policy<Name,
                       ndnSIM::smart_pointer_payload_traits<EntryImpl<ContentStoreImplSpecial<Policy>>,
                                                            Entry>,
                       Policy> {
public:
  typedef ndnSIM::
    trie_with_policy<Name, ndnSIM::smart_pointer_payload_traits<EntryImpl<ContentStoreImplSpecial<Policy>>,
                                                                Entry>,
                     Policy> super;

  typedef EntryImpl<ContentStoreImplSpecial<Policy>> entry;

  static TypeId
  GetTypeId();

  ContentStoreImplSpecial(){};
  virtual ~ContentStoreImplSpecial(){};
...
template<class Policy>
LogComponent
  ContentStoreImplSpecial<Policy>::g_log = LogComponent(("ndn.cs." + Policy::GetName()).c_str());

template<class Policy>
TypeId
ContentStoreImplSpecial<Policy>::GetTypeId()
{
  static TypeId tid =
    TypeId(("ns3::ndn::cs::" + Policy::GetName()).c_str())
      .SetGroupName("Ndn")
      .SetParent<ContentStore>()
      .AddConstructor<ContentStoreImplSpecial<Policy>>()
      .AddAttribute("MaxSize",
                    "Set maximum number of entries in ContentStore. If 0, limit is not enforced",
                    StringValue("100"), MakeUintegerAccessor(&ContentStoreImplSpecial<Policy>::GetMaxSize,
                                                             &ContentStoreImplSpecial<Policy>::SetMaxSize),
                    MakeUintegerChecker<uint32_t>())

      .AddTraceSource("DidAddEntry",
                      "Trace fired every time entry is successfully added to the cache",
                      MakeTraceSourceAccessor(&ContentStoreImplSpecial<Policy>::m_didAddEntry));
...
template<class Policy>
shared_ptr<Data>
ContentStoreImplSpecial<Policy>::Lookup(shared_ptr<const Interest> interest)
{
...
template<class Policy>
bool
ContentStoreImplSpecial<Policy>::Add(shared_ptr<const Data> data)
{
...
template<class Policy>
void
ContentStoreImplSpecial<Policy>::Print(std::ostream& os) const
{
...
template<class Policy>
void
ContentStoreImplSpecial<Policy>::SetMaxSize(uint32_t maxSize)
{
...
template<class Policy>
uint32_t
ContentStoreImplSpecial<Policy>::GetMaxSize() const
{
...
template<class Policy>
uint32_t
ContentStoreImplSpecial<Policy>::GetSize() const
{
  return this->getPolicy().size();
}

/**
* @brief Gets the first item in a cache?
*/
template<class Policy>
Ptr<Entry>
ContentStoreImplSpecial<Policy>::Begin()
{
...
/**
* @brief Method that always returns 0 - fairly useless! Perhaps used in subclass
*/
template<class Policy>
Ptr<Entry>
ContentStoreImplSpecial<Policy>::End()
{
  return 0;
}

/**
* @brief Gets next content-store entry
* It does so by iterating through them until it finds a non-zero payload. It requires a starting place, 'from' where it starts iterating
*/
template<class Policy>
Ptr<Entry>
ContentStoreImplSpecial<Policy>::Next(Ptr<Entry> from)
{
  if (from == 0)
    return 0;

  // Call the Super classes parent_trie (which uses a recursive iterator?) and get the item at entry 'from'
  typename super::parent_trie::recursive_iterator item(*StaticCast<entry>(from)->to_iterator()),
    end(0);

  // Iterate over the items until you find one with a non-zero payload
  ...
  // If you are at the end of the cache return End? otherwise return the payload
  if (item == end)
    return End();
  else
    return item->payload();
...
#endif // NDN_CONTENT_STORE_IMPL_SPECIAL_H_
