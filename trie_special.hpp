...
#ifndef TRIE_SPECIAL_H_
#define TRIE_SPECIAL_H_
...
template<typename FullKey, typename PayloadTraits, typename PolicyHook>
inline std::ostream&
operator<<(std::ostream& os, const trie_special<FullKey, PayloadTraits, PolicyHook>& trie_special_node);

template<typename FullKey, typename PayloadTraits, typename PolicyHook>
bool
operator==(const trie_special<FullKey, PayloadTraits, PolicyHook>& a,
           const trie_special<FullKey, PayloadTraits, PolicyHook>& b);

template<typename FullKey, typename PayloadTraits, typename PolicyHook>
std::size_t
hash_value(const trie_special<FullKey, PayloadTraits, PolicyHook>& trie_special_node);

///////////////////////////////////////////////////
// actual definition
//
template<class T, class NonConstT>
class trie_special_iterator;

template<class T>
class trie_special_point_iterator;
...
public:
  ...
  typedef trie_special* iterator;
  typedef const trie_special* const_iterator;

  typedef trie_special_iterator<trie, trie_special> recursive_iterator;
  typedef trie_special_iterator<const trie_special, trie_special> const_recursive_iterator;

  typedef trie_special_point_iterator<trie> point_iterator;
  typedef trie_special_point_iterator<const trie_special> const_point_iterator;

  typedef PayloadTraits payload_traits;

  inline trie_special(const Key& key, size_t bucketSize = 1, size_t bucketIncrement = 1)
    : key_(key)
  ...
  {
    recursive_iterator trie_specialNode(this);
    recursive_iterator end(0);

    while (trieNode != end) {
      if (cond(*trieNode)) {
        trie_specialNode = recursive_iterator(trieNode->erase());
      }
      trie_specialNode++;
    }
  }
  ...
  // actual entry
  friend bool operator==<>(const trie_special<FullKey, PayloadTraits, PolicyHook>& a,
                           const trie_special<FullKey, PayloadTraits, PolicyHook>& b);
  ...
  {
    trie_special* trie_specialNode = this;

    BOOST_FOREACH (const Key& subkey, key) {
      typename unordered_set::iterator item = trie_specialNode->children_.find(subkey);
      if (item == trie_specialNode->children_.end()) {
        trie_special* newNode = new trie_special(subkey, initialBucketSize_, bucketIncrement_);
        // std::cout << "new " << newNode << "\n";
        newNode->parent_ = trie_specialNode;

        if (trieNode->children_.size() >= trie_specialNode->bucketSize_) {
          trie_specialNode->bucketSize_ += trie_specialNode->bucketIncrement_;
          trie_specialNode->bucketIncrement_ *= 2; // increase bucketIncrement exponentially

          buckets_array newBuckets(new bucket_type[trieNode->bucketSize_]);
          trie_specialNode->children_.rehash(bucket_traits(newBuckets.get(), trie_specialNode->bucketSize_));
          trie_specialNode->buckets_.swap(newBuckets);
        }

        std::pair<typename unordered_set::iterator, bool> ret =
          trie_specialNode->children_.insert(*newNode);

        trie_specialNode = &(*ret.first);
      }
      else
        trie_specialNode = &(*item);
    }

    if (trieNode->payload_ == PayloadTraits::empty_payload) {
      trie_specialNode->payload_ = payload;
  ...
  /**
   * @brief Removes payload (if it exists) and if there are no children, prunes parents trie_special
   */
  inline iterator
  erase()
  {
  ...
  prune()
  {
    if (payload_ == PayloadTraits::empty_payload && children_.size() == 0) {
      ...
      trie_special* parent = parent_;
      parent->children_
        .erase_and_dispose(*this,
                           trie_special_delete_disposer()); // delete this; basically, committing a suicide
  ...
  find(const FullKey& key)
  {
    ...
    BOOST_FOREACH (const Key& subkey, key) {
      ...
      if (item == trie_specialNode->children_.end()) {
        ...
        if (trieNode->payload_ != PayloadTraits::empty_payload)
          foundNode = trie_specialNode;
      }
    }

    return std::make_tuple(foundNode, reachLast, trie_specialNode);
  }
   ...
   * @returns end() or a valid iterator pointing to the trie_special leaf (order is not defined, enumeration
   * )
   */
  inline iterator
  find()
  {
    ...
    typedef trie_special<FullKey, PayloadTraits, PolicyHook> trie_special;
    for (typename trie_special::unordered_set::iterator subnode = children_.begin();
  ...
  find_if(Predicate pred)
  {
    ...
    for (typename trie_special::unordered_set::iterator subnode = children_.begin();
         subnode != children_.end(); subnode++)
    // BOOST_FOREACH (const trie_special &subnode, children_)
    {
...
private:
  ...
  struct trie_special_delete_disposer {
    void
  ...
  friend std::ostream& operator<<<>(std::ostream& os, const trie_special& trie_special_node);
...
private:
  ...
  // necessary typedefs
  typedef trie_special self_type;
  ...
  template<class T, class NonConstT>
  friend class trie_special_iterator;

  template<class T>
  friend class trie_special_point_iterator;
  ...
  trie_special* parent_; // to make cleaning effective
};
...
operator<<(std::ostream& os, const trie_special<FullKey, PayloadTraits, PolicyHook>& trie_special_node)
{
  os << "# " << trie_special_node.key_ << ((trie_node.payload_ != PayloadTraits::empty_payload) ? "*" : "")
     << std::endl;
  typedef trie_special<FullKey, PayloadTraits, PolicyHook> trie_special;

  for (typename trie_special::unordered_set::const_iterator subnode = trie_special_node.children_.begin();
       subnode != trie_special_node.children_.end(); subnode++)
  // BOOST_FOREACH (const trie_special &subnode, trie_special_node.children_)
  {
    os << "\"" << &trie_node << "\""
       << " [label=\"" << trie_special_node.key_
...
trie<FullKey, PayloadTraits, PolicyHook>::PrintStat(std::ostream& os) const
{
  ...
  typedef trie_special<FullKey, PayloadTraits, PolicyHook> trie_special;
  for (typename trie_special::unordered_set::const_iterator subnode = children_.begin();
       subnode != children_.end(); subnode++)
  // BOOST_FOREACH (const trie_special &subnode, children_)
  {
...
class trie_special_iterator {
public:
  trie_special_iterator()
    : trie_special_(0)
  {
  }
  trie_special_iterator(typename Trie::iterator item)
    : trie_special_(item)
  {
  }
  trie_special_iterator(Trie& item)
    : trie_special_(&item)
  {
  }
  ...
  trie_special_iterator<Trie, NonConstTrie>&
  operator++(int)
  {
    if (trie_->children_.size() > 0)
      trie_special_ = &(*trie_->children_.begin());
    else
      trie_special_ = goUp();
    return *this;
  }
...
private:
  ...
  goUp()
  {
    if (trie_->parent_ != 0) {
      ...
      item++;
      if (item != trie_special_->parent_->children_.end()) {
        return &(*item);
      }
      else {
        trie_special_ = trie_special_->parent_;
        return goUp();
      }
    }
    else
...
public:
  trie_special_point_iterator()
    : trie_special_(0)
  {
  }
  trie_special_point_iterator(typename Trie::iterator item)
    : trie_special_(item)
  {
  }
  trie_special_point_iterator(Trie& item)
  {
    if (item.children_.size() != 0)
      trie_special_ = &*item.children_.begin();
    else
      trie_special_ = 0;
  }
  ...
  trie_special_point_iterator<Trie>&
  operator++(int)
  {
    if (trie_->parent_ != 0) {
      set_iterator item = trie_special_->parent_->children_.iterator_to(*trie_);
      item++;
      if (item == trie_special_->parent_->children_.end())
        trie_special_ = 0;
      else
